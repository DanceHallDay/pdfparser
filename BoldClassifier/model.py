...
class PatchEmbeddingBlock(nn.Module):
    def __init__(
        self, feature_extraction_model=None, num_words: int = 8, emb_size: int = 64
    ):
        super(PatchEmbeddingBlock, self).__init__()
        self.projection = nn.Sequential(
            Rearrange(
                "b n c h w -> (b n) c h w"
            ),  # batch_size, num_words, chanels, height, width
            feature_extraction_model
            if feature_extraction_model is not None
            else self.__default_FE_model(emb_size),  # resnet18
            Rearrange(
                "(b n) e -> b n e", e=emb_size, n=num_words
            ),  # batch_size, num_words, emb_size
        )
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(BoldClassifier, self).__init__(
            ...
            *[
                TransformerBlock(num_heads, emb_size, forward_expansion, **kwargs)
